...
bool is_valid_addition(vector<char> &prefix, char next, int available)
{
    int smallest = -1;
...
int count(vector<char> &prefix, int available, int len)
{
    if (available == 0)
    {
        // cout << prefix << endl;
...
int main()
{
    ...
    for (int i = 0; i < len; i++)
    {
        // cout << (1 << i); - couts: 1248 (i.e. powers of 2)
        all |= 1 << i;
    }
    // all = (2^i)-1 - efficient way
    // cout << all;
    ...
    for (char c : prefix_s)
    {
        char i = c - 'A'; // finds difference between A and character - represents it as a string
        // cout << c << " " << (int)i << " ";
        prefix.push_back(i);
